...
export default function ContactForm() {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    if (!key) { setResult("❌ Chýba NEXT_PUBLIC_WEB3FORMS_KEY v .env.local"); return; }
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="pt-2 flex items-center gap-4">
          ...
          {result && (
            <p className={`text-sm ${result.includes("❌") ? "text-red-400" : "text-emerald-400"}`}
              role="status" aria-live="polite">{result}</p>
          )}
        </div>
      </form>
            ...
            >
              ...
              >
                <motion.svg viewBox="0 0 120 120" className="mx-auto mb-6" width={90} height={90}>
                  <motion.circle cx="60" cy="60" r="50" fill="none" stroke="rgb(16 185 129)" strokeWidth="8"
                    initial={{ pathLength: 0 }} animate={{ pathLength: 1 }}
                    transition={{ duration: 0.6, ease: "easeInOut" }} />
                  <motion.path d="M38 62 L54 78 L84 46" fill="none" stroke="rgb(16 185 129)"
                ...
                <button onClick={() => setShowThanks(false)}
                  className="cursor-pointer mt-8 rounded-full px-6 py-3 text-sm font-semibold bg-emerald-500 text-black hover:bg-emerald-400 transition">
                  Pokračovať
                </button>
              </motion.div>
            </motion.div>
